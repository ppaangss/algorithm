...
집계 함수(`COUNT`, `SUM`, `AVG` 등)와 항상 같이 다닌다.
...
왼쪽 테이블의 데이터는 무조건 다 보여주고, 오른쪽 테이블에 매칭되는 게 있으면 붙이고 없으면 `NULL`로 표시한다.
...
같은 테이블을 두 번 불러와서 조인하는 기법이다. 

보통 카테고리-상위 카테고리 관계나 직원-상급자 관계처럼 계층 구조를 가진 데이터를 다룰 때 쓴다.
